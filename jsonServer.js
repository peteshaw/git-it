...
var parsetime = function (m) {
    var retVal = {};
    retVal.hour = m.hour();
    retVal.minute = m.minute();
    retVal.second = m.second();
    return retVal;
}
var unixtime = function(m) {
    var retVal = {};
    retVal.unixtime = Number(m.format('x'));
    return retVal;
}
...
var server = http.createServer(function (request, response) {
    ...
    //you can access the properties by name, i.e. 
    
    //console.log(req);
    var time = moment(req.query.split('=')[1]);


    if (req.pathname == '/api/unixtime') {
        retVal = unixtime(time);
    } else {
        retVal = parsetime(time);
    }

    var sTime = JSON.stringify(retVal);
...
# HTTP JSON API SERVER (Exercise 13 of 13)  
 ...
 ## HINTS  
  ...
  here:  
  file:///home/ubuntu/workspace/node_modules/learnyounode/node_apidoc/url.ht  
  ml  
